function results = getResultsOGStudy(SMatrix,params,groups,maxPerturb,plotFlag,indivFlag)
...
%             (nanmean(forgetB3Data(end-29:end-10,:))-nanmean(forgetB4Data(4:8,:)))./nanmean(forgetB3Data(end-29:end-10,:))];
        ...
        adaptAllData=adaptData.getParamInCond(params,{'adaptation','readaptation'});
...
%             results.Washout2.indiv=[g*ones(nSubs,1) washout2];            
        ...
        for p=1:length(params)     
            ...
            results.Strides2SS.indiv.(params{p})(end+1:end+nSubs,1:2)=[g*ones(nSubs,1) Strides2SS(:,p)];
            results.PerForget.indiv.(params{p})(end+1:end+nSubs,1:2)=[g*ones(nSubs,1) perforget(:,p)];
